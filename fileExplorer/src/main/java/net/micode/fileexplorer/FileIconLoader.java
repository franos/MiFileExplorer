...
public class FileIconLoader implements Callback {
    ...
    /**
     * ImageView和对应图像ID的映射，注意：ID在图像加载请求开始前可能改变
    ...
    /**
     * 保证在同一时间只发送一个MESSAGE_PHOTOS_NEEDED实例
    ...
    /**
     * 加载图像到指定的View中，如果图像已经缓存，则可以马上显示；否则，发送一个请求从数据库加载图像
    ...
    /**
     * 检查图像当前是否已有缓存，是，则载入View中，否则将图像状态变为NEEDED
    ...
    private boolean loadCachedIcon(ImageView view, String path, FileCategory cate) {
        ...
        if (holder == null) {
            ...
            //如果不能载入图像到View中意味着软引用已经被GC了，需要重新载入图像
