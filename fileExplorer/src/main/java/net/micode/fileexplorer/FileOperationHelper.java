...
public class FileOperationHelper {
    ...
    /**
     * 文件操作进度 FileViewInteractionHub实现了该接口
     */
    ...
    private void CopyFile(FileInfo f, String dest) {
        ...
        if (file.isDirectory()) {//如果是文件夹
            ...
            // directory exists in destination, rename it如果存在同名文件夹，加后缀
            ...
            for (File child : file.listFiles(mFilter)) {
                if (!child.isHidden() && Util.isNormalFile(child.getAbsolutePath())) {
                    CopyFile(Util.GetFileInfo(child, mFilter, Settings.instance().getShowDotAndHiddenFiles()), destPath);//递归
                }
            }
        } else {
