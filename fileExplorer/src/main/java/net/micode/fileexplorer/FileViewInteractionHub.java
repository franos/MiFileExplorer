...
public class FileViewInteractionHub implements IOperationProgressListener {
    ...
    public FileViewInteractionHub(IFileInteractionListener fileViewListener) {
        assert (fileViewListener != null);//断言
    ...
    /**
     * 是否显示文件夹CheckBox
     * 当复制和剪切的时候，底部会显示粘贴|取消按钮，此时CheckBox不显示
     * @return
     */
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.button_operation_cancel:
                    ...
                    break;
                case R.id.current_path_pane://切换目录
    ...
    protected void onNavigationBarClick() {
        if (mDropdownNavigation.getVisibility() == View.VISIBLE) {//切换
            showDropdownNavigation(false);
        } else {
            ...
            if (list.getChildCount() > 0)
                showDropdownNavigation(true);//显示下三角
    ...
    public boolean onOperationUpLevel() {
        showDropdownNavigation(false);//关闭层级列表
        //FileViewActivity已实现该接口
        if (mFileViewListener.onOperation(GlobalConsts.OPERATION_UP_LEVEL)) {//return false
            return true;
        }

        if (!mRoot.equals(mCurrentPath)) {
            mCurrentPath = new File(mCurrentPath).getParent();//将当前的目录改为父目录并刷新列表
            refreshFileList();
            return true;//返回True,表明返回事件已处理，不需要转交给基类
        }
    ...
    public boolean onBackPressed() {
        if (mDropdownNavigation.getVisibility() == View.VISIBLE) {//如果快捷导航栏处于打开状态，则关闭它
            mDropdownNavigation.setVisibility(View.GONE);
        } else if (isInSelection()) {//如果处于多选状态，清除该状态
            clearSelection();
        } else if (!onOperationUpLevel()) {
            return false;//没有处理，返回给基类处理
        }
        return true;
    }
